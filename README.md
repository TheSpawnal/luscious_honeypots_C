...
Build a small, deliberately vulnerable C binary, and breaking it. A classic stack overflow/craft the input, overwrite the saved return address, seize the instruction pointer, all on your own machine of course Sir, in your own lab, with no victim but a program you wrote to die. Then the part that turns vandalism into craft: we switch each mitigation on in turn and watch your exploit perish against it: the stack canary, NX, ASLR, PIE, _FORTIFY_SOURCE. Need to feel precisely why each defense exists,  stood on both sides of the breach. Wwrecking fire in service of the wall. 

Aldric de Jacquelin Dulphe
